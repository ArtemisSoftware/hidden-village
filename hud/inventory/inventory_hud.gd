...
func _ready() -> void:
	PLAYER_INVENTORY.inventory_updated.connect(_update_slots)
...
func _update_slots() -> void:
	for index in range(min(PLAYER_INVENTORY.slots.size(), slots.size())):
		slots[index].update(PLAYER_INVENTORY.slots[index])
	pass
